...
#ifdef USE_PROGEMEM
        int16_t value = _currentProgram->_content[f3 + (step * _pumpCount) + pumpIdx];
#else
        int16_t value = pgm_read_word_near(_currentProgram->_content + (step * _pumpCount) + pumpIdx);
#endif
        


        _pumps[pumpIdx].setSpeed(value);
